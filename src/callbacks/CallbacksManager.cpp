...
int CallbacksManagerClass::createCallbackFromJson(JsonObject observableInfo,
                                                  JsonObject callback) {
  if (observableInfo.size() == 0) {
    LOGGER.error(CALLBACKS_MANAGER_TAG,
  ...
  if (strcmp(type, STATE_TYPE) == 0) {
    return addCallback<String>(SmartThing.getDeviceState(name),
                               CallbacksFactory::build<String>(callback));
  } else if (strcmp(type, SENSOR_TYPE) == 0) {
    return addCallback<int16_t>(SmartThing.getSensor(name),
                                CallbacksFactory::build<int16_t>(callback));
  }

  LOGGER.error(CALLBACKS_MANAGER_TAG, "Unkown observable object type: %s",
               type);
  return -1;
}

template <class T>
int CallbacksManagerClass::addCallback(const ConfigurableObject<T> *obj,
                                       Callback<T> *callback) {
  if (obj == nullptr) {
    LOGGER.error(CALLBACKS_MANAGER_TAG,
  ...
  _callbacksCount++;
  LOGGER.info(CALLBACKS_MANAGER_TAG, "Added new callback(id=%d) for %s [%s]",
              callback->getId(), obj->type, obj->name);
  return callback->getId();
}

template <typename T>
Watcher<T> *CallbacksManagerClass::getWatcherOrCreate(
  ...
  if (watcher == nullptr) {
    LOGGER.debug(CALLBACKS_MANAGER_TAG, "Creating new watcher for %s [%s]",
                 obj->type, obj->name);
    ...
    LOGGER.info(CALLBACKS_MANAGER_TAG, "Added new watcher for %s [%s]",
                obj->type, obj->name);
  } else {
    LOGGER.debug(CALLBACKS_MANAGER_TAG, "Watcher for %s [%s] already exists!",
                 obj->type, obj->name);
  }
  return watcher;
}
...
template <>
Watcher<int16_t> *CallbacksManagerClass::createWatcher(
    const ConfigurableObject<int16_t> *obj) {
  return new SensorWatcher((Sensor *)obj);
}

template <>
Watcher<String> *CallbacksManagerClass::createWatcher(
...
bool CallbacksManagerClass::deleteCallback(const char *type, const char *name,
                                           int id) {
...
bool CallbacksManagerClass::updateCallback(DynamicJsonDocument doc) {
  ...
  LOGGER.error(CALLBACKS_MANAGER_TAG, "Observable type [%s] not supported!",
               type);
  return false;
}

template <typename T>
bool CallbacksManagerClass::updateCallback(List<Watcher<T>> *list,
                                           const char *name,
                                           JsonObject callbackObject) {
  if (!callbackObject.containsKey("id")) {
    LOGGER.error(CALLBACKS_MANAGER_TAG,
                 "Id value in callback object is missing!");
    return false;
  }
  ...
  if (callback->isReadonly()) {
    LOGGER.error(CALLBACKS_MANAGER_TAG,
                 "Callback %d for observable [%s] is readonly!", id, name);
    return false;
  }

  if (callbackObject.containsKey("trigger")) {
    callback->setTriggerValue(callbackObject["trigger"]);
    callback->setTriggerDisabled(
        callbackObject["trigger"].as<String>().isEmpty());
  }
  ...
  LOGGER.info(CALLBACKS_MANAGER_TAG,
              "Callback id=%d for observable [%s] was updated!", id, name);
  return true;
}

template <typename T>
Callback<T> *CallbacksManagerClass::getCallbackFromWatcherList(
    List<Watcher<T>> *list, const char *name, int id) {
  Watcher<T> *watcher = getWatcherByObservableName(list, name);
  if (watcher == nullptr) {
    LOGGER.warning(CALLBACKS_MANAGER_TAG,
  ...
  if (callback == nullptr) {
    LOGGER.warning(CALLBACKS_MANAGER_TAG,
                   "Can't find callback id=%d for observable [%s]", id, name);
    return nullptr;
  }
  return callback;
}
...
template <typename T>
bool CallbacksManagerClass::deleteCallbackFromList(List<Watcher<T>> *list,
                                                   const char *name, int id) {
  if (name == nullptr || strlen(name) == 0) {
    LOGGER.error(CALLBACKS_MANAGER_TAG, "Name of observable is missing!");
    return false;
  }
  LOGGER.debug(CALLBACKS_MANAGER_TAG,
               "Trying to delete observable [%s]'s callback id=%d", name, id);
  Watcher<T> *watcher = getWatcherByObservableName(list, name);
  ...
  _callbacksCount--;
  LOGGER.warning(CALLBACKS_MANAGER_TAG,
                 "Callback № %d of observable [%s] was deleted", id, name);
  if (watcher->haveCallbacks()) {
    return true;
  }
  LOGGER.debug(CALLBACKS_MANAGER_TAG,
               "No callbacks left for observable [%s], removing watcher!",
               name);
  if (!list->remove(watcher)) {
    return false;
  }
  delete watcher;
  LOGGER.warning(CALLBACKS_MANAGER_TAG, "Watcher for observable [%s] removed!",
                 name);
  return true;
}

template <typename T>
Watcher<T> *CallbacksManagerClass::getWatcher(
...
template <typename T>
Watcher<T> *CallbacksManagerClass::getWatcherByObservableName(
...
template <typename T>
void CallbacksManagerClass::collectInfo(List<Watcher<T>> *list,
                                        JsonArray *array) {
  list->forEach(
...
DynamicJsonDocument CallbacksManagerClass::allCallbacksToJson(
  ...
  LOGGER.debug(CALLBACKS_MANAGER_TAG, "DynamicJsonDoc size for callbacks = %d",
               size);
...
DynamicJsonDocument CallbacksManagerClass::getObservableCallbacksJson(
...
template <typename T>
DynamicJsonDocument CallbacksManagerClass::getObservableCallbacksJsonFromList(
    List<Watcher<T>> *list, const char *name) {
  if (name == nullptr || strlen(name) == 0) {
    LOGGER.error(CALLBACKS_MANAGER_TAG, "Name of observable is missing!");
  } else {
...
DynamicJsonDocument CallbacksManagerClass::getCallbackJsonById(const char *type,
                                                               const char *name,
                                                               int id) {
  if (strcmp(type, SENSOR_WATCHER_TYPE) == 0) {
...
template <typename T>
DynamicJsonDocument CallbacksManagerClass::getCallbackJsonFromList(
    List<Watcher<T>> *list, const char *name, int id) {
  if (name == nullptr || strlen(name) == 0) {
    LOGGER.error(CALLBACKS_MANAGER_TAG, "Name of observable is missing!");
  } else {
    ...
    if (watcher != nullptr) {
      ...
      if (callback != nullptr) {
        ...
        LOGGER.warning(CALLBACKS_MANAGER_TAG,
                       "Can't find callback with id [%d] in watcher for [%s]",
                       id, name);
      }
    } else {
      LOGGER.warning(CALLBACKS_MANAGER_TAG,
