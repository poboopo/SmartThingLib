...
class Watcher {
 public:
  ...
  DynamicJsonDocument toJson(bool ignoreReadOnly, bool shortJson) {
    ...
    DynamicJsonDocument callbacks =
        getObservableCallbacksJson(ignoreReadOnly, shortJson);
  ...
  bool addCallback(Callback::Callback<T> *callback) {
    ...
    if (callback->isReadonly()) {
      ...
      LOGGER.error(WATCHER_TAG, "Callback with id=%d already exists!",
                   callback->getId());
      return false;
    }
  ...
  bool removeCallback(int id) {
    ...
    if (callback == nullptr) {
      LOGGER.error(WATCHER_TAG,
                   "Failed to remove callback - can't find callback with id %d",
                   id);
      return false;
    }
    if (callback->isReadonly()) {
  ...
  bool haveCallbacks() { return _callbacks.size() != 0; }

  uint8_t callbacksCount() { return _callbacks.size(); }
