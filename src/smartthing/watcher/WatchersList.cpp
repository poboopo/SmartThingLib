...
bool WatchersList::registerSensorWatcher(const Sensor * sensor, Callback callback) {
    if (sensor == nullptr) {
...
bool WatchersList::registerDeviceStateWatcher(const DeviceState * state, Callback callback) {
    if (state == nullptr) {
